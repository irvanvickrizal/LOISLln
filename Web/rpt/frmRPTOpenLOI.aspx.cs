...
public partial class rpt_frmRPTOpenLOI : System.Web.UI.Page
{
    ...
    private void BindData(int pgIndex = 0)
    {
        ...
        gvListLOI.DataSource = loiControllerr.report_loi_open_getdata(subconid, periodfrom, periodto);
    ...
    protected void btnDownloadList_Click(object sender, EventArgs e)
    {
        ...
        string periodfrom = txtperiodfrom.Value;
        string periodto = txtperiodto.Value;
        DataTable dtResult = loiControllerr.report_loi_open_detail_getdata(subconid, periodfrom, periodto, "", "", 0);
        if (dtResult.Rows.Count > 0)
            ExportToExcel("LOI_Open_" + string.Format("{0:ddMMMyyy}", DateTime.Now), dtResult, GeneralConfig.Def_sheetname);
    }
    ...
    protected void gvListLOI_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        ...
        {
            Response.Redirect("~/rpt/frmRPTOpenLOIDetail.aspx?RequestId=" + e.CommandArgument.ToString());
        }
    }
}