...
public partial class Dashboard_frmApprovalCDM : System.Web.UI.Page
{
    ...
    protected void btnSubmitReject_Click(object sender, EventArgs e)
    {
        ...
        int RequestId = int.Parse(Request.QueryString["RequestId"]);
        if (loiControllerr.LOI_Update_Approval_Status(RequestId, false, "CDM", txtReasonRejection.Value))
        {
            try
            {
            ...
            {
                ScriptManager.RegisterStartupScript(this.Page, Page.GetType(), Guid.NewGuid().ToString(), "alert('LOI has been Rejected Successfully'); location.href = '../dashboard/frmListLOICDM.aspx';", true);
            }
        }
        else
        {
    ...
    {
        int RequestId = int.Parse(Request.QueryString["RequestId"]);
        if (loiControllerr.LOI_Update_Approval_Status(RequestId, true, "CDM", ""))
        {
            try
            {
                SendEmail(RequestId);
            ...
            {
                ScriptManager.RegisterStartupScript(this.Page, Page.GetType(), Guid.NewGuid().ToString(), "alert('LOI has been Approved Successfully'); location.href = '../dashboard/frmListLOICDM.aspx';", true);
            }
        }
        else
        {
    ...
    private void SendEmail(int ReqId)
    {
        ...
        if (dtReceiverMail.Rows.Count > 0)
        {
            ...
            loiControllerr.sendMailApproval(dtReceiverMail, htmltabledetail, "MB Approval", ContentSession.FULLNAME, ReqId, "MB");
