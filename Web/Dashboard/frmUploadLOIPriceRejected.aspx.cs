...
public partial class Dashboard_frmUploadLOIPriceRejected : System.Web.UI.Page
{
    ...
    protected void btnDownloadTemplate_Click(object sender, EventArgs e)
    {
        ...
        string filename = string.Concat(TempPath, "List LOI Rejected.xlsx");
        try
        {
        ...
        try
        {
            using (XLWorkbook workBook = new XLWorkbook(path))
            {
                ...
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    workSheet.Range("O3:O" + (i + 3).ToString()).Merge();
                    workSheet.Range("O3:O" + (i + 3).ToString()).Style.Alignment.Horizontal = XLAlignmentHorizontalValues.Left;
                    workSheet.Range("O3:O" + (i + 3).ToString()).Style.Alignment.Vertical = XLAlignmentVerticalValues.Top;
                }
    ...
    protected void lblErrorUpload_Click(object sender, EventArgs e)
    {
        ...
        string filename = string.Concat(TempPath, "List_Failed.xlsx");
        try
        {
            File.Copy(filenameSource, filename, true);
