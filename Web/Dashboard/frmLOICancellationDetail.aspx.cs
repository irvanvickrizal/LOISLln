...
public partial class Dashboard_frmLOICancellationDetail : System.Web.UI.Page
{
    ...
    protected void btnCancel_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            foreach (GridViewRow gvr in gvLOIData.Rows)
            {
                ...
                if (ChkReview != null)
                {
                    ...
                    {
                        if (loiControllerr.LOI_Detail_Update_Approval_Status(workpackageid, LOI_Detail_ID, RequestId, "PCM Cancel"))
                        {
                            successcount += 1;
                        }
                        else
            ...
            if (failedcount == 0 && successcount > 0)
            {
                ...
                ScriptManager.RegisterStartupScript(this.Page, Page.GetType(), Guid.NewGuid().ToString(), "alert('LOI has been cancelled Successfully'); location.href = '../dashboard/frmLOICancellation.aspx';", true);
            }            
            else if (successcount > 0 && failedcount > 0)
            {
                //sendemail(gvr.Cells[4].Text, gvr.Cells[9].Text, gvr.Cells[7].Text, password);
                ScriptManager.RegisterStartupScript(this.Page, Page.GetType(), Guid.NewGuid().ToString(), "alert('" + successcount.ToString() + " LOI has been cancelled Successfully and " + failedcount.ToString() + " Failed to cancelled LOI');", true);
            }
            else if (successcount == 0)
            {
                ScriptManager.RegisterStartupScript(this.Page, Page.GetType(), Guid.NewGuid().ToString(), "alert('Failed to cancelled LOI');", true);
            }
        ...
        //int RequestId = int.Parse(Request.QueryString["RequestId"]);
        ////if (loiControllerr.LOI_Update_Approval_Status(RequestId, true, "PCM Cancel", ""))
        //{
        //    ScriptManager.RegisterStartupScript(this.Page, Page.GetType(), Guid.NewGuid().ToString(), "alert('LOI has been cancelled Successfully'); location.href = '../dashboard/frmLOIClossingPending.aspx';", true);
        //}
        //else
        //{
        //    ScriptManager.RegisterStartupScript(this.Page, Page.GetType(), Guid.NewGuid().ToString(), "alert('Failed to cancelled LOI');", true);
