...
public partial class Dashboard_frmCompletedDataUpload : System.Web.UI.Page
{
    ...
    {
        string filenameSource = Server.MapPath("~/template_file/Template_Upload_FM.xlsx");
        ...
        string filename = string.Concat(TempPath, "Template_Upload_FM.xlsx");

        try
        {
            File.Copy(filenameSource, filename, true);
            DataTable dt = loiControllerr.SubconName_getList();
            DataTable dtSOW = loiControllerr.master_sow_getList_detail();
            DataTable dtRegion = loiControllerr.get_region_by_userid();
            insertTemplateFileValue(filenameSource, filename, dt, dtSOW, dtRegion);
                ...
                if (dtLOI.Rows.Count > 0)
                {
                    using (System.Transactions.TransactionScope scope = new System.Transactions.TransactionScope())
                    {
                        try
                        {
                            ...
                            int RequestId = loiControllerr.LOI_Upload_by_FM(Subcon_Id, scope_type_id);
                            if (RequestId > 0)
                            {
                            ...
                            if (errorcount == 0)
                            {
                                if (loiControllerr.validate_loi_scope(RequestId) == 1)
                                {
                                    try
                                    {
                                        SendEmail(RequestId);
                                ...
                                else
                                {
                                    ...
                                    ScriptManager.RegisterStartupScript(this.Page, Page.GetType(), Guid.NewGuid().ToString(), "alert('there is different scope type in one loi');", true);
                                }
                            }
                            else
                            {
    ...
    //protected void btnDownloanSubcon_Click(object sender, EventArgs e)
    //{
    //    string filenameSource = Server.MapPath("~/template_file/Template_List_Subcon.xlsx");

    //    string TempPath = string.Concat("~/TemporaryFile/", DateTime.Now.ToString("ddMMyyyy"), ContentSession.FULLNAME, "/");
    //    TempPath = Server.MapPath(TempPath);
    //    if (!Directory.Exists(TempPath)) Directory.CreateDirectory(TempPath);

    //    string filename = string.Concat(TempPath, "List_Subcon.xlsx");
    //    try
    //    {
    //        File.Copy(filenameSource, filename, true);
    ...
    public bool insertTemplateFileValue(string path, string newpath, DataTable dt, DataTable dtSOW, DataTable dtRegion)
    {
        bool result = false;
        try
        {
            using (XLWorkbook workBook = new XLWorkbook(path))
            {
                ...
                {
                    workSheet.Cell(i + 2, 1).Value = dt.Rows[i]["SCon_Name"].ToString();
                }
    ...
    protected void lblErrorUpload_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            DataTable dt = (DataTable)Session["dtLOIError"];
