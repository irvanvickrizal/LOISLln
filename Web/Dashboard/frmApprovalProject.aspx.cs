...
public partial class Dashboard_frmApprovalProject : System.Web.UI.Page
{
    ...
    protected void btnSubmitReject_Click(object sender, EventArgs e)
    {
        ...
        int RequestId = int.Parse(Request.QueryString["RequestId"]);
        if (loiControllerr.LOI_Update_Approval_Status(RequestId, false, "PBM", txtReasonRejection.Value))
        {
            try
            {
            ...
            {
                ScriptManager.RegisterStartupScript(this.Page, Page.GetType(), Guid.NewGuid().ToString(), "alert('LOI has been Rejected Successfully'); location.href = '../dashboard/frmListLOIPBMCDM.aspx';", true);
            }
        }
        else
        {
    ...
    {
        int RequestId = int.Parse(Request.QueryString["RequestId"]);
        if (loiControllerr.LOI_Update_Approval_Status(RequestId, true, "PBM", ""))
        {
            try
            {
                SendEmail(RequestId);
            ...
            {
                ScriptManager.RegisterStartupScript(this.Page, Page.GetType(), Guid.NewGuid().ToString(), "alert('LOI has been Approved Successfully'); location.href = '../dashboard/frmListLOIPBMCDM.aspx';", true);
            }
        }
        else
        {
    ...
    private void SendEmail(int ReqId)
    {
        ...
        if (dtReceiverMail.Rows.Count > 0)
        {
            ...
            loiControllerr.sendMailApproval(dtReceiverMail, htmltabledetail, "CDM Approval", ContentSession.FULLNAME, ReqId, "CDM");
