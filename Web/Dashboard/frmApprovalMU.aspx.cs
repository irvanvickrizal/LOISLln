...
public partial class Dashboard_frmApprovalMU : System.Web.UI.Page
{
    ...
    protected void btnApprove_Click(object sender, EventArgs e)
    {
        ...
        if (loiControllerr.LOI_Update_Approval_Status(RequestId, true, ApproverRole, ""))
        {
            ...
            {
                ScriptManager.RegisterStartupScript(this.Page, Page.GetType(), Guid.NewGuid().ToString(), "alert('LOI has been Approved Successfully'); location.href = '../dashboard/frmListLOIMU.aspx';", true);
            }
        }
        else
        {
    ...
    //private void BindLinkOverDue()
    //{
    //    int OverdueCount = loiControllerr.LOI_get_UploadedCount("LOI Overdue");
    //    if (OverdueCount > 0)
    //    {
    //        lbLOIOverdue.Text = "Total LOI Over Due: " + OverdueCount.ToString();
    //        lbLOIOverdue.Enabled = true;
    //    }

    //}

    protected void lbLOIOverdue_Click(object sender, EventArgs e)
    {
    ...
    private void SendEmail(int ReqId)
    {
        ...
        if (dtReceiverMail.Rows.Count > 0)
        {
            ...
            string loistatus = ContentSession.RoleID == 6 ? "Approved" : "MU Approval";
            loiControllerr.sendMailApproval(dtReceiverMail, htmltabledetail, loistatus, ContentSession.FULLNAME, ReqId, "MU");
