...
public partial class Dashboard_frmCheckingSupportingDoc : System.Web.UI.Page
{
    ...
    protected void btnSubmit_Click(object sender, EventArgs e)
    {
        ...
        {
            int RequestId = int.Parse(Request.QueryString["RequestId"]);
            if (loiControllerr.LOI_Update_SupDoc_Availibility(RequestId))
            {
                string querystr = Request.QueryString["isRejected"] == "1" ? "location.href = '../dashboard/frmCPMApprovalRejected.aspx?RequestId=" + Request.QueryString["RequestId"] + "';" : "location.href = '../dashboard/frmApproval.aspx?RequestId=" + Request.QueryString["RequestId"] + "';";
                ScriptManager.RegisterStartupScript(this.Page, Page.GetType(), Guid.NewGuid().ToString(), "alert('Submit checking data Succcess, next task budget approval');" + querystr, true);
                //try
                //{
                //    SendEmail(RequestId);
                //}
                ...
                //finally
                //{
                //    string querystr = Request.QueryString["isRejected"] == "1" ? "location.href = '../dashboard/frmListLOICPMRejected.aspx';" : "location.href = '../dashboard/frmListLOICPM.aspx';";
    ...
    {
        int RequestId = int.Parse(Request.QueryString["RequestId"]);
        gvSupDoc.DataSource = loiControllerr.cpm_checking_document_getall(RequestId);
        gvSupDoc.DataBind();
    }
    ...
    private void SendEmail(int ReqId)
    {
        ...
        if (dtReceiverMail.Rows.Count > 0)
        {
            ...
            loiControllerr.sendMailApproval(dtReceiverMail, htmltabledetail, "CPM Approval", ContentSession.FULLNAME, ReqId, "CPM");
